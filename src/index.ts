...
const scrape = async () => {
    ...
    for (let searchIndex = 0; searchIndex < searches.rowCount; searchIndex++) {
        ...
        const dueDate = new Date().setTime(
            createdAt + search.timeLimit * MILLISECONDS_PER_DAY
        );
        ...
        if (now > dueDate) {
            ...
            if (
                now <
                new Date(search.kijijiLastSearchedAt).getTime() +
                    search.kijijiFrequency * MILLISECONDS_PER_HOUR
            ) {
            ...
            if (
                now <
                new Date(search.fbmarketplaceLastSearchedAt).getTime() +
                    search.fbmarketplaceFrequency * MILLISECONDS_PER_HOUR
            ) {
    ...
    if (
        now <
        new Date(settings.bunz.lastSearchedAt).getTime() +
            settings.bunz.frequency * MILLISECONDS_PER_HOUR
    ) {
    ...
    if (
        now <
        new Date(settings.gcsurplus.lastSearchedAt).getTime() +
            settings.gcsurplus.frequency * MILLISECONDS_PER_HOUR
    ) {
