...
const navigateToResults =
    ...
    async (searchTerm: string): Promise<void> => {
        await page.goto(
            `https://www.facebook.com/marketplace/category/search/?query=${encodeURIComponent(
                searchTerm
            )}`
        );
...
const filterResultLinks = (links: string[]): string[] =>
    filterUnique(links.map(stripURLParams));

const scrollDown = (page: Page): Promise<void> =>
    page.evaluate((_) => window.scrollBy(VIEWPORT_HEIGHT, window.innerHeight));

const parsePrice = (text: string): number =>
    Math.round(
        parseFloat(
            text.replace("$", "").replace(",", "").replace("C", "").trim()
        )
    );

const scrapeResult =
    (page: Page, navigationPromise: Promise<HTTPResponse | null>) =>
    async (url: string): Promise<Result> => {
        ...
        // Title
        text = await scrapeInnerText(
            ".j83agx80 > div > .dati1w0a > div > .d2edcug0"
        );
        result.title = text.trim();

        // Price
        text = await scrapeInnerText(
            "div > .dati1w0a > .aov4n071 > div > .d2edcug0 > span > span"
        );
        result.oldPrice = parsePrice(text);

        const priceText: string = await scrapeInnerText(
            "div > .dati1w0a > .aov4n071 > div > .d2edcug0"
        );
        result.price = parsePrice(priceText.replace(text, ""));

        // Location
        text = await scrapeInnerText(
            ".dati1w0a > .aov4n071 > .sjgh65i0 > div > .d2edcug0 > a"
        );
        result.location = text.trim();

        // Listed At
        const listedAtText: string = await scrapeInnerText(
            ".dati1w0a > .aov4n071 > .sjgh65i0 > div > .d2edcug0"
        );
        const listedAtTrimmed: string = listedAtText
            .replace(text, "")
            .replace("Listed", "")
            .replace("in", "")
            .replace("ago", "")
        ...
        let listedAtQuantity: number =
            listedAtTrimmed.indexOf("a ") === 0 ||
            listedAtTrimmed.indexOf("an ") === 0
                ? 1
                : parseInt(listedAtTrimmed);
        ...
        // Condition
        text = await scrapeInnerText(
            ".n99xedck:nth-child(2) > .j83agx80 > .qzhwtbm6 > .d2edcug0 > .d2edcug0"
        );
        ...
        text = await scrapeInnerText(
            ".rq0escxv > .rq0escxv > .ii04i59q > div > .d2edcug0"
        );
...
export const scrape =
    ...
    async (search: Search): Promise<void> => {
        try {
            // Puppeteer
        ...
        `);
            ...
            const prevSearchResults = await db.query(
                "SELECT * FROM searches_fbmarketplace_results WHERE searchId=$1",
                [search.id]
            );

            try {
                loop: while (
                    resultLinks.length < search.resultLimit &&
                    noNewResultsCount < 4 &&
                    waitPeriod < 30000
                ) {
                    ...
                    timesScrolled++;
                    while (
                        newResultLinks.length === resultLinks.length &&
                        noNewResultsCount < 4 &&
                        waitPeriod < 30000
                    ) {
                ...
                if (noNewResultsCount >= 4 || waitPeriod > 30000) {
                    console.log(
                        `infinite scroll limits reached, no new results: ${noNewResultsCount}, waitPeriod: ${waitPeriod}, results: ${resultLinks.length}`
                    );
                }
            } catch (error: any) {
            ...
            for (
                let resultIndex = 0;
                resultIndex < resultLinks.length;
                resultIndex++
            ) {
                try {
                    const result = await scrapeResult(
                        page,
                        navigationPromise
                    )(resultLinks[resultIndex] as string);
